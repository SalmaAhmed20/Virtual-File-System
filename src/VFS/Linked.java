...
public class Linked {
    ...
    private static ArrayList<Integer> store_Indexes = new ArrayList<Integer>();
    ...
    int  Allocate (int FSize) {
        ...
        while(count !=FSize) {
            pointer = (int)(Math.random()* DiskSize)+1;
            if(Blocks[pointer] == ('0') ) {
                Array.setChar (Blocks,pointer,'1');
                FreeBlocks--;
                count++;
                store_Indexes.add(pointer);
            }
        }
        i+=FSize;
        System.out.println("After Allocate " + String.valueOf (Blocks)) ;
        for(int j=0;j<store_Indexes.size();j++) {
            System.out.print(store_Indexes.get(j)+" ");
            if (j==store_Indexes.size()-1) {
                System.out.print("\n");
            }
        }
        count=0;
        FreeBlocks=DiskSize-store_Indexes.size();
        return store_Indexes.get(i-FSize);
    }
    Boolean Deallocat( int start,int FSize) {
        int var;
        if(FreeBlocks==0) {
            System.out.println("This File doesnot allocated before");
        }else {

            for(int a=start;a<FSize;a++) {
                if ( Blocks[store_Indexes.get(a)] == ('1') ) {
                    var=store_Indexes.get(a);
                    Array.setChar (Blocks,var,'0');
                    this.FreeBlocks++;
    ...
    Boolean CreateFile(String path, int Size) {
        ...
        if (dir != null) // if the file doesn't exist  then you can add it
        {
            ...
            if ( this.Existfile (F.getName (),dir.getFiles ()) ==-1) //doesn't exist before
            {
                start=Allocate (Size);
                ...
                else
                {
                    ...
                    F.getAllocatedBlocks ()[0][1] =store_Indexes.get(i-1);
    ...
    void DeleteFile (String path) {
        ...
        if(dir != null) {
            ...
            if(idx == - 1) {
                ...
                int size = Math.abs (store_Indexes.indexOf(files.getAllocatedBlocks ()[ 0 ][ 1 ])+1);
                Deallocat(Start,size);
                if(! this.Deallocat(store_Indexes.indexOf(Start),size)) System.out.println ("Error");
    ...
    void storeToVFS () {
        try{
            FileWriter fWriter = new FileWriter ("Linked.vfs");
    ...
    void Retrive (String path,int startidx,int end) {
        ...
        if(dir != null) // if the file doesn't exist  then you can add it
        {
            ...
            {
                start = Allocate( store_Indexes.indexOf(end)-store_Indexes.indexOf(startidx));
                if(start == - 1) {
                    return;
                }else {
                    if(dir.getFiles () != null) {
    ...
    {
        try{
            FileReader fR= new FileReader("Linked.vfs");
    ...
    public static void main (String[] args) {
        ...
        l1.CreateFolder("root/folder1");
        System.out.println ("--------------------------------");
        l1.CreateFile("root/folder1/file.txt", 10);
        System.out.println ("--------------------------------");
        l1.CreateFile("root/file1.txt", 5);
        System.out.println ("--------------------------------");
        l1.DeleteFile("root/folder1/file.txt");
        //l1.DeleteFolder("root/folder2");
        System.out.println ("--------------------------------");
        l1.DisplayDiskStatus();
        //System.out.println ("--------------------------------");
        //l1.DisplayDiskStructure(l1.root);
        System.out.println ("--------------------------------");
        l1.storeToVFS();
    }
}